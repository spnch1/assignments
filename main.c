...
int main(void) {
    ...
    double a=0, b=0, c=0; //Sides of the triangle - N
    ...
    while (1) {
        while (a + b <= c || a + c <= b || b + c <= a) {
            ...
            if (a + b <= c || a + c <= b || b + c <= a) {
                printf("Invalid input. These sides do not form a triangle. Press press Enter to try again...\n");
                fflush(stdin);
                getch();
            }
        }
        ...
        while (DP == 0) {
            printf("Now enter the number of decimal points [0,12] for the results:\n");
            while (scanf("%hhu", &DP) != 1) {
                printf("Invalid input. Please try again...\n");
            }
        }
